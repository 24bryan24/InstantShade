...
const BRAND = {
  ...
  colors: {
    navy: "bg-[#1B365D]", // Deep Navy from business card
    orange: "text-[#F97316]",
    orangeBg: "bg-[#F97316]",
...
const InstantShadeLogo = ({ className = "h-10 w-10" }) => (
  <svg viewBox="0 0 100 100" className={className} fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    {/* Blue Swoosh */}
    <path 
      d="M75 20C85 20 90 35 90 50C90 70 80 85 60 90C65 88 80 75 80 50C80 35 75 25 70 20H75Z" 
      fill="#1B365D" 
    />
  </svg>
);
...
const INITIAL_PRICING = {
  // Base installation price per unit by type
  types: [
  ...
  // Surcharges per unit based on brand complexity
  brands: [
  ...
  // Flat fees or multipliers
  fees: {
    highLadder: 45, // Flat fee for bringing/using high ladder
    specialSurface: 20, // Per window fee for tile/metal
    minimumTrip: 100 // Minimum service charge
  }
};
...
// 1. Navigation
const Navbar = ({ setView, currentView, cartCount }) => {
          ...
          <div className="hidden md:block">
            <div className="ml-10 flex items-baseline space-x-8">
              ...
              <button 
                ...
                className="bg-orange-500 hover:bg-orange-600 text-white px-6 py-2.5 rounded-full font-bold uppercase tracking-wide transition-all shadow-md transform hover:scale-105"
              >
                Book Now
              </button>
            </div>
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 md:py-32 relative z-10">
      <div className="lg:w-2/3">
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <button 
            onClick={() => setView('quote')}
            className="flex items-center justify-center bg-orange-500 hover:bg-orange-600 text-white text-lg px-8 py-4 rounded-lg font-bold transition-all shadow-lg hover:shadow-orange-500/25"
          >
...
// 4. Quote Calculator & Booking Form (The Core Feature)
const QuoteForm = ({ pricing, onSubmitRequest }) => {
  const [items, setItems] = useState([
    { id: 1, type: pricing.types[0].id, brand: pricing.brands[0].id, width: '', height: '', qty: 1 }
  ]);
  ...
  const addItem = () => {
    setItems([...items, { id: Date.now(), type: pricing.types[0].id, brand: pricing.brands[0].id, width: '', height: '', qty: 1 }]);
  };
  ...
  const calculateTotal = () => {
    ...
    items.forEach(item => {
      ...
      const itemTotal = (typePrice + brandSurcharge) * parseInt(item.qty || 0);
      subtotal += itemTotal;
        ...
        <div className="p-8">
          <div className="flex justify-between items-end border-b border-slate-100 pb-6 mb-6">
            ...
            <div className="text-right">
              <p className="text-sm text-slate-500">Includes labor & supplies</p>
            </div>
          </div>
          ...
          <div className="space-y-4 mb-8">
             <div className="flex justify-between text-sm">
               ...
               <span className="font-medium">{customer.name} ({customer.zip})</span>
             </div>
             <div className="flex justify-between text-sm">
             ...
             <div className="flex justify-between text-sm">
               ...
               <span className="font-medium">{items.reduce((acc, curr) => acc + parseInt(curr.qty), 0)}</span>
             </div>
          </div>

          <button 
            onClick={confirmBooking}
            className={`w-full ${BRAND.colors.navy} hover:bg-[#2a4575] text-white text-lg font-bold py-4 rounded-xl shadow-lg transition-transform transform hover:scale-[1.02]`}
          >
            Confirm & Schedule Appointment
          </button>
          <button 
            onClick={() => setShowSummary(false)}
            className="w-full mt-4 text-slate-500 font-medium hover:text-slate-800"
          >
            Edit Details
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 md:p-8 space-y-8">
          ...
          <div className="space-y-4">
            ...
            {items.map((item, index) => (
              <div key={item.id} className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end bg-slate-50 p-4 rounded-xl border border-slate-100 relative group">
                ...
                <div className="md:col-span-3">
                  ...
                  <select 
                    required
                    className="w-full rounded-lg border-slate-300 shadow-sm focus:border-orange-500 focus:ring-orange-500 text-sm"
                    value={item.type}
            ...
            <button 
              type="button"
              onClick={addItem}
              className={`text-sm font-bold ${BRAND.colors.navyText} hover:text-orange-600 flex items-center gap-2 mt-2`}
            >
               ...
               <div className="space-y-3">
                 <label className="flex items-center gap-3 p-3 border border-slate-200 rounded-lg cursor-pointer hover:bg-slate-50">
                    <input 
                      ...
                      checked={conditions.highLadder}
                      onChange={(e) => setConditions({...conditions, highLadder: e.target.checked})}
                    />
                    <div>
                 ...
                 <label className="flex items-center gap-3 p-3 border border-slate-200 rounded-lg cursor-pointer hover:bg-slate-50">
                    <input 
                      type="checkbox" 
                      className="rounded text-orange-500 focus:ring-orange-500 h-5 w-5"
                      checked={conditions.metalTile}
                      onChange={(e) => setConditions({...conditions, metalTile: e.target.checked})}
                    />
                    <div>
                 ...
                 <div>
                   ...
                   <div className="relative">
                     ...
                     <input 
                        type="date" 
                        required
                        className="pl-10 w-full rounded-lg border-slate-300 shadow-sm focus:border-orange-500 focus:ring-orange-500"
                        value={schedule.date}
          ...
          <div className="border-t border-slate-200 pt-6">
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
               <input 
                 ...
                 required
                 className="rounded-lg border-slate-300 shadow-sm focus:border-orange-500 focus:ring-orange-500"
                 value={customer.name}
          ...
          <div className="pt-4">
             <button 
              type="submit" 
              className="w-full bg-orange-500 hover:bg-orange-600 text-white font-bold text-xl py-4 rounded-xl shadow-lg shadow-orange-500/30 transition-all transform hover:scale-[1.01]"
             >
               Calculate Quote & Schedule
             </button>
...
const AdminDashboard = ({ pricing, setPricing, setView }) => {
  ...
  const handlePriceChange = (category, id, field, value) => {
    ...
    if (category === 'fees') {
      updated.fees[id] = parseFloat(value);
    } else {
      ...
      <div className="min-h-[60vh] flex items-center justify-center">
        <div className="bg-white p-8 rounded-xl shadow-lg border border-slate-200 w-full max-w-md">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <input 
              type="password" 
              className="w-full rounded-lg border-slate-300 p-3" 
              placeholder="Enter Password" 
              value={password}
              onChange={(e) => setPassword(e.target.value)}
            />
            <button className={`w-full ${BRAND.colors.navy} text-white font-bold py-3 rounded-lg hover:bg-[#2a4575]`}>
              Access Dashboard
            </button>
    ...
    <div className="max-w-5xl mx-auto py-8">
      <div className="flex justify-between items-center mb-8">
        ...
        <div className="flex gap-4">
          <button onClick={() => setView('home')} className="flex items-center gap-2 text-slate-600 hover:text-slate-900">
              ...
              <div key={type.id} className="flex justify-between items-center">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <input 
                    type="number" 
                    value={type.price}
                    onChange={(e) => handlePriceChange('types', type.id, 'price', e.target.value)}
                    className="w-24 rounded border-slate-300 text-right p-1"
                  />
                </div>
              </div>
            ))}
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
             ...
             <div className="space-y-4">
               <div className="flex justify-between items-center">
                  <span className="text-slate-600">High Ladder Flat Fee</span>
                  <div className="flex items-center gap-2">
                    <span className="text-slate-400">$</span>
                    <input 
                      type="number" 
                      value={localPricing.fees.highLadder}
                      onChange={(e) => handlePriceChange('fees', 'highLadder', null, e.target.value)}
                      className="w-24 rounded border-slate-300 text-right p-1"
                    />
                  </div>
               </div>
               <div className="flex justify-between items-center">
                  <span className="text-slate-600">Metal/Tile Drill (Per Window)</span>
                  <div className="flex items-center gap-2">
                    <span className="text-slate-400">$</span>
                    <input 
                      type="number" 
                      value={localPricing.fees.specialSurface}
                      onChange={(e) => handlePriceChange('fees', 'specialSurface', null, e.target.value)}
                      className="w-24 rounded border-slate-300 text-right p-1"
                    />
                  </div>
               </div>
               <div className="flex justify-between items-center">
                  <span className="text-slate-600">Minimum Trip Charge</span>
                  <div className="flex items-center gap-2">
                    <span className="text-slate-400">$</span>
                    <input 
                      type="number" 
                      value={localPricing.fees.minimumTrip}
                      onChange={(e) => handlePriceChange('fees', 'minimumTrip', null, e.target.value)}
                      className="w-24 rounded border-slate-300 text-right p-1"
                    />
                  </div>
               </div>
                ...
                <div key={brand.id} className="flex justify-between items-center">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <input 
                      ...
                      onChange={(e) => handlePriceChange('brands', brand.id, 'surcharge', e.target.value)}
                      className="w-20 rounded border-slate-300 text-right p-1"
                    />
                  </div>
                </div>
              ))}
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="grid md:grid-cols-4 gap-8">
        <div className="col-span-1 md:col-span-2">
          <div className="flex items-center gap-2 mb-4 text-white">
            <div className="bg-white p-1 rounded-md">
               <InstantShadeLogo className="h-8 w-8" />
            </div>
          ...
          <div className="flex items-center gap-2 text-orange-400 font-semibold">
            ...
            <span>{BRAND.owner}, Owner/Operator</span>
          </div>
        </div>
        <div>
        ...
        <div>
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              <Phone className="h-4 w-4 text-orange-400" /> <a href={`tel:${BRAND.phone}`} className="hover:text-white transition-colors">{BRAND.phone}</a>
            </div>
            <div className="flex items-center gap-2">
              <MapPin className="h-4 w-4 text-orange-400" /> {BRAND.email}
            </div>
            <div className="flex items-center gap-2">
              <Clock className="h-4 w-4 text-orange-400" /> Mon-Sun: 7am - 9pm
            </div>
          </div>
        </div>
      </div>
      <div className="border-t border-slate-700 mt-12 pt-8 text-center text-sm">
        &copy; {new Date().getFullYear()} {BRAND.name}. All rights reserved. {BRAND.slogan}
      </div>
    </div>
  </footer>
);
...
const App = () => {
  const [currentView, setCurrentView] = useState('home'); // home, quote, admin
          ...
          <div className="bg-orange-500 text-white py-12 relative overflow-hidden">
            ...
            <div className="max-w-4xl mx-auto px-4 text-center relative z-10">
              ...
              <button 
                ...
                className={`bg-white ${BRAND.colors.navyText} font-bold py-3 px-8 rounded-full shadow-lg hover:bg-slate-100 transition-colors uppercase tracking-wider`}
              >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm">
          <div className="bg-white rounded-2xl p-8 max-w-md w-full text-center shadow-2xl">
            ...
            <p className="text-slate-600 mb-8">
              We have received your installation request. <strong>{BRAND.owner}</strong> or one of our team members will call you shortly at {BRAND.phone} to confirm.
            </p>
            <button 
              onClick={closeSuccessModal}
              className={`w-full ${BRAND.colors.navy} text-white font-bold py-3 rounded-xl hover:bg-[#2a4575] transition-colors`}
            >
              Return Home
            </button>
          </div>
